function fetchPokemon(pokemonId) {
    ...
    return fetch(`${URL}/${ENDPOINT}/${pokemonId}`)
      .then((response) => {
        if (!response.ok) {
  ...
  fetchPokemon(5).then(console.log).catch(console.log);
  ...
  function handleSearch(event) {
    ...
    const queryValue = form.elements.query.value.trim().toLowerCase(); // Исправлено здесь
